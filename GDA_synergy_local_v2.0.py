...
normalized_means_pivot = df2.pivot(index='Row Drug Concentration', columns='Column Drug Concentration', values='Normalized Mean')

# Create a pivot table for Bliss independence
bliss_independence_pivot = bliss_df.pivot(index='Row Drug Concentration', columns='Column Drug Concentration', values='Bliss Independence')
...
x_values = np.where(x_values == 0, 1e-10, x_values)
y_values = np.where(y_values == 0, 1e-10, y_values)
...
# Automatically determine tick values and labels for x and y axes
x_tickvals = np.logspace(np.floor(np.log10(np.min(x_values[x_values > 0]))), np.ceil(np.log10(np.max(x_values))), num=10)
y_tickvals = np.logspace(np.floor(np.log10(np.min(y_values[y_values > 0]))), np.ceil(np.log10(np.max(y_values))), num=10)

x_ticktext = [f'{val:.1e}' if val != 1e-10 else '0' for val in x_tickvals]
y_ticktext = [f'{val:.1e}' if val != 1e-10 else '0' for val in y_tickvals]
